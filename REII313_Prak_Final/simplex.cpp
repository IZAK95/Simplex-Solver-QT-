...
void Simplex::CalculateSimplex()
{
    ...
    qDebug()<<"Answers for the Constraints of variables"<<endl;
    ...
    for(int i=0;i< A.size(); i++){  //every basic column has the values, get it form B array
        ...
        if(count0 == rows -1 ){
            ...
            qDebug()<<"variable"<<index+1<<": "<<B[index]<<endl;  //every basic column has the values, get it form B array
        }
        else{
            qDebug()<<"variable"<<index+1<<": "<<0<<endl;
    ...
    qDebug()<<""<<endl;
    qDebug()<<"maximum value: "<<maximum<<endl;  //print the maximum values
}
