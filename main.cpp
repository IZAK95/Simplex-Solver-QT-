...
int i, j;
cout << "\n" << ++counter << ". Tableau " << mes << ":" << endl;
//printf("\n%d. Tableau %s:\n", ++counter, mes);
nl(70);

printf("%-6s%5s", "col:", "b[i]");
for(j=1;j<tab->n; j++) { cout << "  x" << j << ","; }
cout << "\n";
//for(j=1;j<tab->n; j++) { printf(" x%d,", j); } printf("\n");

for(i=0;i<tab->m; i++) {
if (i==0) cout << "max:"; else
cout << "b" << i;
for(j=0;j<tab->n; j++) {
...
cout << "Most negative column in row[0] is col " << pivot_col << " = " << lowest << ".\n";
...
if (min_ratio == -1)
return -1; // Unbounded.
cout << "Found pivot A[" << pivot_row <<"," << pivot_col << "], min positive ratio=" << min_ratio <<" in row=" << pivot_row <<".\n";
